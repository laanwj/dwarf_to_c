...
class GenerableStruct(Struct):
    def __init__(self, tpname, fields, name=None,
        ...
        *aligned_prime_to* is a list of integers.
        ...
        Struct.__init__(self, tpname, fields, name, padded_bytes - bytes)
...
class Union(Declarator):
    """An union declarator."""

    def __init__(self, tpname, fields, name=None, pad_bytes=0):
        """Initialize the union declarator.
        *tpname* is the name of the union, while *name* is the
...
def iterator_pre_post(i, pre='', post=''):
    '''Prepend *pre* to first line, append *post* to last line in iterator *i*'''
    i = iter(i)
    try:
        prevline = pre + i.next()
    except StopIteration:
...
def iterator_append(i, j):
    '''
    Like itertools.chain, except that the last line of *i* is appended to the first line of *j*

    >>> list(iterator_append(["a","b"],["c","d"]))
    ['a', 'bc', 'd']
    '''
    prevline = None
    for line in i:
        if prevline is not None:
            yield prevline
        prevline = line
    pre = prevline if prevline is not None else ''
...
class Enum(Declarator):
    """An enumeration declarator."""

    def __init__(self, tpname, items, name=None):
        """Initialize the enumeration declarator.
        *tpname* is the name of the enumeration, while *name* is the
        name used for the declarator. 
        *items* is a list of :class:`EnumItem` instances.
        """
        self.tpname = tpname
        self.items = items 
        self.name = name
    ...
    def get_decl_pair(self):
        def get_tp():
            ...
            for i in self.items[:-1]:
                comment = i.comment.generate() if i.comment is not None else []
                for i_line in iterator_append(iterator_pre_post(i.generate(), "  ", ", "), comment):
                    yield i_line
            for i in self.items[-1:]:
                comment = i.comment.generate() if i.comment is not None else []
                for i_line in iterator_append(iterator_pre_post(i.generate(), "  ", " "), comment):
...
class EnumItem(Generable):
    """An enumeration item"""

    def __init__(self, key, value=None, comment=None):
        """Initialize the enumeration item.
        *key* is the key of the enumeration item, while
        *value* is an optional value.
        *comment* is a :class:`Comment` instance, or None."""
        self.key = key
        self.value = value
        self.comment = comment

    def generate(self):
        yield "%s = %s" % (self.key, self.value)
